...
namespace Arya.Framework.Data
{
    ...
    internal class TaxAttValueCalculator
    {
        ...
        public string ProcessCalculatedAttribute(string expression, int maxResultLength)
        {
            ...
            try
            {
                ...
                return RxWhiteSpace.Replace(tokenValue.Trim(), " ");
            }
            catch (Exception)
            {
                return "<Error>";
        ...
        private class Token
        {
            ...
            public Token(int id, Sku sku, string expression)
            {
                ...
                else if (expression.StartsWith("[") && expression.EndsWith("]"))
                {
                    ...
                    {
                        _tokenType = TokenType.AttributeValueOnly;
                        expression = expression.Replace(SubAttributeValueOnlyExpression, string.Empty).Trim();
                    }
                    else
                        _tokenType = TokenType.Attribute;
                }
                else if (expression.StartsWith("\"") && expression.EndsWith("\""))
                    _tokenType = TokenType.Constant;
